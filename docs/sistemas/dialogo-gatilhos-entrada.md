...
- `MazeEntryController` (cliente), ao receber `MazeEntrance1..4`: cria uma parede invisível **local** (filha da Camera) atrás da zona, uma cortina de névoa de partículas sobre a abertura, um `BlurEffect` que pulsa 14 → 0 em 3,5 s e o tween de Lighting em 6 s. O efeito é por jogador; o servidor não fecha nada para os outros.
